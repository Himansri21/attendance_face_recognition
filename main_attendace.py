...
class MainApp(App):
    ...
    def Attendance(self):
        ...
        try:
            ...
            # Update attendance: ensure attendance CSV has today's date column and update status
            attendance_file = os.path.join(self.Dir, 'Attendance', 'Attendance.csv')
            try:
            ...
            # Retrieve user name from users.csv if possible
            try:
                users_csv = os.path.join(self.Dir, 'list', 'users.csv')
                df_users = pd.read_csv(users_csv)
                name = df_users.loc[df_users['id'] == user_id, 'name'].iloc[0]
            ...
            # Ensure today's date column exists; default to "Absent"
            if date_str not in df_att.columns:
                df_att[date_str] = "Absent"

            # If the user is not yet in the attendance file, add a new row with default "Absent"
            ...
            # Update the attendance status based on recognition
    ...
    def dataset(self):
        ...
        try:
            ...
            if face_id not in df_att['id'].values:
                new_row = pd.DataFrame([{'id': face_id, 'name': name}])
                # For any additional attendance columns, default to "Absent"
