...
class MainApp(App):
    ...
    def Attendance(self):
        ...
        try:
            ...
            face_numbers = 5
            blink = 0
            no_face_frame_count = 0   # Counts consecutive frames with no face detected
            ...
            while self.running:
                ...
                # Only update blink count if a face is detected
                if len(faces) > 0:
                    no_face_frame_count = 0  # Reset counter if face is found
                    if len(eyes) < 2:
                        blink += 1
                else:
                    # Increase no-face counter; if too many frames with no face, reset blink count
            ...
            # Update attendance CSV: add a column for today's date (default "Absent") and update user's status
            attendance_file = os.path.join(self.Dir, 'Attendance', 'Attendance.csv')
            try:
